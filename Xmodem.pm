  ...
  if (!$fstatus_open) {
    ...
    if ( $message{type} eq Device::SerialPort::Xmodem::Constants::nul() ) {
      ...
      if ($message_status) {
        ...
        # Only want to push this on if it is the next block number
        # If the previous ACK sent (to confirm successful packet) was not
        # not received by the sender end, then the sender re-sends the
        # packet. This PERL module doesn't seem to have logic to determine
        # if this is re-transmission of a successfully sent packet.
        #
        # Note 1: We still want to send an Ack whatever the situation.
        #
        # Note 2: A lost NCK will not always trigger following condition
        #         because the last successful block was two blocks ago
        #
...
sub receive_message {
  ...
  do {
    ...
    # If we lose packets, we might miss START of packet...
    # and we want to get ALL the rest of the end of the packet
    # i.e. we don't want to end receiving the data from the serial
    # port when we device drip-feeds this info. So we give ourselves
    # a time limit...
    if (($shortmessage == 1) && (time > $receive_start_time + 2)) {
	$done = 1;
    }
