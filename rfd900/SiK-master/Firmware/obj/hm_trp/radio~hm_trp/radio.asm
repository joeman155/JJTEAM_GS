...
;	radio/radio.c:721: clear_status_registers();
	lcall	_clear_status_registers
;	radio/radio.c:731: register_write(EZRADIOPRO_GPIO0_CONFIGURATION, 0x15);	// RX state (output)
	mov	a,#0x15
	push	acc
	mov	dpl,#0x0B
	lcall	_register_write
	dec	sp
;	radio/radio.c:732: register_write(EZRADIOPRO_GPIO1_CONFIGURATION, 0x12);	// TX state (output)
	mov	a,#0x12
	push	acc
	mov	dpl,#0x0C
	lcall	_register_write
