...
;------------------------------------------------------------
;	radio/parameters.c:295: flash_write_byte(uint16_t address, uint8_t c) __reentrant __critical
;	-----------------------------------------
...
00103$:
	...
	mov	r6,dpl
	mov	r7,dph
;	radio/parameters.c:297: PSCTL = 0x01;				// set PSWE, clear PSEE
	mov	_PSCTL,#0x01
;	radio/parameters.c:298: FLKEY = 0xa5;
	mov	_FLKEY,#0xA5
;	radio/parameters.c:299: FLKEY = 0xf1;
	mov	_FLKEY,#0xF1
;	radio/parameters.c:300: *(uint8_t __xdata *)address = c;	// write the byte
	mov	dpl,r6
	mov	dph,r7
	mov	a,sp
	add	a,#0xfd
	...
	mov	a,@r0
	movx	@dptr,a
;	radio/parameters.c:301: PSCTL = 0x00;				// disable PSWE/PSEE
	mov	_PSCTL,#0x00
	pop	psw
	mov	ea,c
	ret
...
;address                   Allocated to registers r6 r7 
;------------------------------------------------------------
;	radio/parameters.c:305: flash_read_byte(uint16_t address) __reentrant
;	-----------------------------------------
;	 function flash_read_byte
;	-----------------------------------------
_flash_read_byte:
;	radio/parameters.c:308: return *(uint8_t __code *)address;
	clr	a
	movc	a,@a+dptr
	mov	dpl,a
	ret
...
;------------------------------------------------------------
;	radio/parameters.c:312: calibration_set(uint8_t idx, uint8_t value) __reentrant
;	-----------------------------------------
;	 function calibration_set
;	-----------------------------------------
_calibration_set:
;	radio/parameters.c:315: if (idx <= BOARD_MAXTXPOWER && value != 0xFF)
	mov	a,dpl
	mov	r7,a
	add	a,#0xff - 0x1E
	jc	00104$
	...
	sjmp	00104$
00118$:
;	radio/parameters.c:318: if (flash_read_byte(FLASH_CALIBRATION_AREA_HIGH + idx) == 0xFF)
	mov	r6,#0x00
	mov	a,#0xDE
	add	a,r7
	mov	r4,a
	...
	pop	ar7
	cjne	r5,#0xFF,00104$
;	radio/parameters.c:320: flash_write_byte(FLASH_CALIBRATION_AREA_HIGH + idx, value);
	mov	a,#0xDE
	add	a,r7
	mov	r7,a
	mov	a,#0xFB
	...
	dec	sp
;	radio/parameters.c:321: return true;
	setb	c
	ret
00104$:
;	radio/parameters.c:324: return false;
	clr	c
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	radio/parameters.c:328: calibration_get(uint8_t level) __reentrant
;	-----------------------------------------
;	 function calibration_get
;	-----------------------------------------
_calibration_get:
	mov	r7,dpl
;	radio/parameters.c:331: uint8_t crc = 0;
	mov	r6,#0x00
;	radio/parameters.c:333: for (idx = 0; idx < FLASH_CALIBRATION_AREA_SIZE; idx++)
	mov	r5,#0x00
00106$:
;	radio/parameters.c:335: crc ^= calibration[idx];
	mov	a,r5
...
00126$:
	jc	00106$
;	radio/parameters.c:338: if (calibration_crc != 0xFF && calibration_crc == crc && level <= BOARD_MAXTXPOWER)
...
00128$:
	...
	add	a,#0xff - 0x1E
	jc	00103$
;	radio/parameters.c:340: return calibration[level];
	mov	a,r7
	mov	dptr,#_calibration
	movc	a,@a+dptr
	mov	dpl,a
	ret
00103$:
;	radio/parameters.c:342: return 0xFF;
	mov	dpl,#0xFF
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	radio/parameters.c:346: calibration_lock() __reentrant
;	-----------------------------------------
;	 function calibration_lock
;	-----------------------------------------
_calibration_lock:
;	radio/parameters.c:349: uint8_t crc = 0;
	mov	r7,#0x00
;	radio/parameters.c:352: if (flash_read_byte(FLASH_CALIBRATION_CRC_HIGH) == 0xFF)
	mov	dptr,#0xFBFD
	push	ar7
	lcall	_flash_read_byte
	mov	r6,dpl
	pop	ar7
	cjne	r6,#0xFF,00105$
;	radio/parameters.c:354: for (idx=0; idx < FLASH_CALIBRATION_AREA_SIZE; idx++)
	mov	r6,#0x00
00106$:
;	radio/parameters.c:356: uint8_t cal = flash_read_byte(FLASH_CALIBRATION_AREA_HIGH + idx);
	mov	ar4,r6
	mov	r5,#0x00
	mov	ar2,r4
	mov	ar3,r5
	...
	pop	ar6
	pop	ar7
;	radio/parameters.c:357: crc ^= cal;
	mov	a,r3
	xrl	ar7,a
;	radio/parameters.c:358: if (cal == 0xFF)
	cjne	r3,#0xFF,00107$
;	radio/parameters.c:360: printf("dBm level %u not calibrated\n",idx);
	push	ar4
	push	ar5
	mov	a,#__str_0
	push	acc
	...
	add	a,#0xfb
	mov	sp,a
;	radio/parameters.c:361: return false;
	clr	c
	ret
00107$:
;	radio/parameters.c:354: for (idx=0; idx < FLASH_CALIBRATION_AREA_SIZE; idx++)
...
00126$:
	jc	00106$
;	radio/parameters.c:366: flash_write_byte(FLASH_CALIBRATION_CRC_HIGH, crc);
	push	ar7
	mov	dptr,#0xFBFD
	lcall	_flash_write_byte
	dec	sp
;	radio/parameters.c:369: flash_write_byte(FLASH_LOCK_BYTE, 0xFE);
	mov	a,#0xFE
	push	acc
	mov	dptr,#0xFBFF
	lcall	_flash_write_byte
	dec	sp
;	radio/parameters.c:370: return true;
	setb	c
	ret
00105$:
;	radio/parameters.c:372: return false;
	clr	c
	ret
	.area CSEG    (CODE)
	.area CONST   (CODE)
